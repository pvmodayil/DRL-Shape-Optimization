...
int main(){
    ...
    // Do prediction & get curve
    //--------------------------------------------------------------------
    Eigen::ArrayXd action = Eigen::ArrayXd::Zero(4);
    int num_pts = 30;
    Eigen::ArrayXd gptsX;
    Eigen::ArrayXd gptsY;
    MSA::getBezierCurve(action, arrangementD.hw_micrstr,arrangementD.hw_arra,num_pts,gptsX,gptsY);

    Eigen::ArrayXd filtered_gptsX = gptsX.segment(1,num_pts-2); // first and last elements are filtered out
    Eigen::ArrayXd filtered_gptsY = gptsY.segment(1,num_pts-2);
    ...
    GA::GeneticAlgorithm ga_problem = GA::GeneticAlgorithm(arrangementD,filtered_gptsY,filtered_gptsX,population_size,num_generations,0.1);
    double noise_scale = 0.1;

    // Result struct
    GA::GeneticResult result = GA::GeneticResult(Eigen::ArrayXd::Zero(num_generations+1), Eigen::VectorXd(gptsY.size() + 3), init_energy); // Empty initialization
    ...
    std::vector<double> result_vectorY(result.best_curve.data(), result.best_curve.data() + result.best_curve.size());
    std::vector<double> result_vectorX(gptsX.data(), gptsX.data() + gptsX.size());
    std::unordered_map<std::string, std::vector<double>> result_data;
    result_data["g_ptsx"] = result_vectorX;
    result_data["g_ptsy"] = result_vectorY;
